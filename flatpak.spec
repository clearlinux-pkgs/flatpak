...
Patch2: 0002-Point-to-real-locale-location.patch
...
%prep
%setup -q -n flatpak-1.16.1
cd %{_builddir}/flatpak-1.16.1
%patch -P 1 -p1
%patch -P 2 -p1
pushd ..
cp -a flatpak-1.16.1 buildavx2
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1748649338
...
export GOAMD64=v2
meson --libdir=lib64 --prefix=/usr --buildtype=plain -Ddocbook_docs=disabled \
-Dgdm_env_file=true \
-Dgtkdoc=disabled \
-Dman=disabled \
...
-Dsystem_dbus_proxy=/usr/bin/xdg-dbus-proxy  builddir
ninja -v -C builddir
GOAMD64=v3
CFLAGS="$CFLAGS -march=x86-64-v3 -Wl,-z,x86-64-v3 " CXXFLAGS="$CXXFLAGS -march=x86-64-v3 -Wl,-z,x86-64-v3 " LDFLAGS="$LDFLAGS -march=x86-64-v3 " meson --libdir=lib64 --prefix=/usr --buildtype=plain -Ddocbook_docs=disabled \
-Dgdm_env_file=true \
-Dgtkdoc=disabled \
-Dman=disabled \
-Dprofile_dir=/usr/share/defaults/etc/profile.d \
-Dseccomp=disabled \
-Dsystem_bubblewrap=/usr/bin/bwrap \
-Dsystem_dbus_proxy=/usr/bin/xdg-dbus-proxy  builddiravx2
...
cp %{_builddir}/flatpak-%{version}/subprojects/variant-schema-compiler/COPYING %{buildroot}/usr/share/package-licenses/flatpak/41bcf91722cf3570c4d65b1b22be6730173f9f7d || :
GOAMD64=v3
...
%files lib
%defattr(-,root,root,-)
/V3/usr/lib64/libflatpak.so.0.11601.0
/usr/lib64/libflatpak.so.0
/usr/lib64/libflatpak.so.0.11601.0
...
/usr/share/package-licenses/flatpak/41bcf91722cf3570c4d65b1b22be6730173f9f7d
